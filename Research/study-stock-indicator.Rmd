...
universe_stocks <- trd_company.tib %>%
    select(stkcd, stkname= stknme, indcd = nindcd, indname = nindnme) %>%
    arrange(indcd, stkcd)


# all industries in investment universe
universe_industry <- universe_stocks %>%
...
**共`r nrow(universe_industry)`个行业, `r nrow(universe_stocks)`只股票**
...
the_indicator <- focus_fields_code[1]
...
top_industries <- industry_indicator_summary %>%
     top_n(top_last_n_industies, median) %>%
     select(indname) 
...
industry_indicator_summary %>%
  ...
  ggplot(aes(x = reorder(indname, median ), y = median)) +
    ...
    geom_label(aes(label = format(median, digits = 2, nsmall = 2)), hjust = TRUE) +
    coord_flip()
...
industry_indicator_summary %>%
  ...
  # arrange(desc(mean)) %>%
  ggplot(aes(x = reorder(indname, desc(median)), y = median)) +
    geom_col() +
    labs(
    ...
    geom_label(aes(label = format(median, digits = 2, nsmall = 2)), hjust = FALSE) +
    coord_flip()
